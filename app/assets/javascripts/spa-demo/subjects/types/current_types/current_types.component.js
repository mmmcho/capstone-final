(function () {
  ...
  angular
      ...
      .component("sdCurrentTypesMap", {
        template: "<div id='map'></div>",
        controller: CurrentTypesMapController,
        bindings: {
          zoom: "@"
        }
      });

  CurrentTypesMapController.$inject = ["$scope", "$q", "$element",
    "spa-demo.geoloc.currentOrigin",
    "spa-demo.geoloc.myLocation",
    "spa-demo.geoloc.Map",
    "spa-demo.types.currentTypes",
    "spa-demo.config.APP_CONFIG"];
  function CurrentTypesMapController($scope, $q, $element, currentOrigin, myLocation, Map, currentTypes, APP_CONFIG) {
    var vm = this;
    ...
    vm.$postLink = function () {
      ...
      $scope.$watch(
          function () {
            return currentTypes.getCurrentImage();
          },
          function (link) {
            if (link) {
      ...
      $scope.$watch(
          ...
          function (marker) {
            if (marker) {
              currentTypes.setCurrentTypeId(marker.thing_id, marker.image_id);
            }
          });
      $scope.$watch(
          function () {
  ...
  CurrentTypesMapController.prototype.updateOrigin = function () {
  ...
  CurrentTypesMapController.prototype.setActiveMarker = function (thing_id, image_id) {
    if (!this.map) {
      return;
  ...
  CurrentTypesMapController.prototype.originInfoWindow = function (location) {
  ...
  CurrentTypesMapController.prototype.thingInfoWindow = function (ti) {
  ...
  CurrentTypesMapController.prototype.imageInfoWindow = function (ti) {
